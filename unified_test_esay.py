...
# -*- coding: utf-8 -*-
"""
run_kvp_unified_repaired.py

Repaired unified Vlasov-Poisson local-velocity parameterization experiments.

Fixes over previous unified script:
  1) Data are generated by an actual split semi-Lagrangian VP solver for 1D1V/2D2V/3D3V
     by default, instead of a prescribed analytic trajectory.
  2) Conv, diffusion-style, and low-rank all use a unified log-density decoder:
        q(v|x,t) = softmax(g(v;x,t)) / prod(dv)
  3) Low-rank uses the successful PI architecture everywhere:
        g(v;x,t) = sum_alpha C_alpha(x,t) Phi_alpha(v)
     Basis/coefficients are fit in log-density space, never density-space.
  4) Data mode and PI mode share the same decoder for each model.
  5) Pure PI uses E computed by spectral Poisson from predicted rho, not a free E state.
  6) Every run/setting updates reports immediately. External data paths are supported.
  7) Full evolution heatmaps are saved for all frames.
